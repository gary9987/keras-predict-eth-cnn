import tensorflow as tf
import pandas as pd
import numpy as np
from imblearn.under_sampling import RandomUnderSampler
from solve_cudnn_error import *
from sklearn.model_selection import train_test_split

import os
import datetime

solve_cudnn_error()

def make_train(data, k_length = 13):

    x_df = []
    y_df = []

    for i in range(0, data.shape[0]- k_length):
        y_df.append(data['strategy'][i])

    data.drop(columns=['strategy'], inplace=True)

    for i in range(0, data.shape[0]- k_length):
        x_df.append(np.rot90(data[i:i+k_length].to_numpy(), 1))

    return np.array(x_df), np.array(y_df)

if __name__ == '__main__':  # For test Class

    window = 80
    data = pd.read_csv('1o2.csv', dtype=np.float)
    data2 = pd.read_csv('2o2.csv', dtype=np.float)
    data3 = pd.read_csv('3o2.csv', dtype=np.float)

    x, y = make_train(data, window)
    x2, y2 = make_train(data2, window)
    x3, y3 = make_train(data3, window)

    orig_shape = x.shape
    orig_shape2 = x2.shape
    orig_shape3 = x3.shape
    print(orig_shape)
    print(orig_shape2)
    print(orig_shape3)

    x = np.reshape(x, (x.shape[0], x.shape[2]*x.shape[1]))
    x2 = np.reshape(x2, (x2.shape[0], x2.shape[2] * x2.shape[1]))
    x3 = np.reshape(x3, (x3.shape[0], x3.shape[2] * x3.shape[1]))

    X_new, Y_new = RandomUnderSampler().fit_resample(x, y)
    X_new2, Y_new2 = RandomUnderSampler().fit_resample(x2, y2)
    X_new3, Y_new3 = RandomUnderSampler().fit_resample(x3, y3)

    X_new = np.reshape(X_new, (X_new.shape[0], orig_shape[1], orig_shape[2], 1))
    Y_new = tf.keras.utils.to_categorical(Y_new, 3)
    print(X_new.shape, Y_new.shape)

    X_new2 = np.reshape(X_new2, (X_new2.shape[0], orig_shape2[1], orig_shape2[2], 1))
    Y_new2 = tf.keras.utils.to_categorical(Y_new2, 3)
    print(X_new2.shape, Y_new2.shape)

    X_new3 = np.reshape(X_new3, (X_new3.shape[0], orig_shape3[1], orig_shape3[2], 1))
    Y_new3 = tf.keras.utils.to_categorical(Y_new3, 3)
    print(X_new3.shape, Y_new3.shape)

    #XTraining, XEval, YTraining, YEval = train_test_split(X_new, Y_new, stratify=Y_new, test_size=0.4, random_state = 68 )  # before model building

    #XTraining, XValidation, YTraining, YValidation = train_test_split(X_new, Y_new, stratify=Y_new, test_size=0.3, random_state = 87 )  # before model building

    model = tf.keras.models.Sequential([
        tf.keras.layers.Conv2D(32, (3, 3), activation='relu', padding='same', input_shape=(orig_shape[1], orig_shape[2], 1)),
        tf.keras.layers.Conv2D(64, (3, 3), activation='relu', padding='same'),
        tf.keras.layers.MaxPooling2D((2, 2), strides=(2, 2)),
        tf.keras.layers.Dropout(0.25),

        tf.keras.layers.Flatten(),
        tf.keras.layers.Dropout(0.5),
        tf.keras.layers.Dense(3, activation=tf.nn.softmax)
    ])
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['categorical_accuracy'])

    #logdir = os.path.join("logs", datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    #tensorboard_callback = tf.keras.callbacks.TensorBoard(logdir, histogram_freq=1)

    #model.fit(XTraining, YTraining, batch_size=20, epochs=100, validation_data=(XValidation, YValidation))
    #model.fit(XTraining, YTraining, batch_size=20, epochs=60, validation_split = 0.15)
    model.fit(X_new, Y_new, batch_size=20, epochs=100, validation_data=(X_new3, Y_new3))

    cost = model.evaluate(X_new2, Y_new2, batch_size=20)
    print("test cost: {}".format(cost))
    model.save('model.h5')


